...
def runGame(grid_size, side_length, mode):
	...
	# store the sequence of code moves to move the player 
	code_sequence = []
	dq = deque() 
	# put start_point for the deque
	dq.append(start_point)
	while carryOn:
		...
		for event in pygame.event.get():# user did something
			if event.type == pygame.QUIT:
				...
				#Pressing the Esc or q Key will quit the game
				if event.key == pygame.K_ESCAPE or event.key == pygame.K_q:
					pygame.quit()


		# CODE CARS CONTROLS ARE HERE
		...
		if (pygame.time.get_ticks() - start_timer > cooldown):
			if keys[pygame.K_RIGHT]:
				code_sequence.append("RIGHT")
				# restart cooldown timer
				start_timer = pygame.time.get_ticks()

				print("CURRENT CODE SEQUENCE: ", code_sequence)

			elif keys[pygame.K_LEFT]:
				code_sequence.append("LEFT")
				# restart cooldown timer
				start_timer = pygame.time.get_ticks()
				...
				print("CURRENT CODE SEQUENCE: ", code_sequence)
			...
			elif keys[pygame.K_r]: # When the user wants to run the code, they will press 'r'
					# get the current point of character

					# save the current position of the character so that it can be restarted when the code is wrong
					saved_character_point = (grid_size-1,grid_size-1)

					""" Move the character for every command in the code block, provided that there are no obstructions
						if there are obstructions, restart the player to the original position.
					"""
					for command in code_sequence:
						...
						if command == "RIGHT":
							# check if the next point is in the maze
							next_point = (current_point[0]+1,current_point[1])
							if not ((current_point[0]+1,current_point[1]) in vertices and (maze.is_edge((current_point,next_point)))):
								player1.shake_character()
								code_sequence = []

								# This is the code to retrace the steps of the character back to the starting position using the deque that stores the
								# previous movements of the character
								print(dq)
								while dq:
									player1.move_character_smooth(dq.pop(), 5)
									time.sleep(0.1)
								dq.append(start_point)
								break
							else:
						...
						# move character up
							...
							next_point = (current_point[0], current_point[1]-1)
							if not ((current_point[0],current_point[1]-1) in vertices and (maze.is_edge((current_point,next_point)))):
								player1.shake_character()
								code_sequence = []
					...
					# restart the list of commands
					code_sequence = []
					print("THIS IS THE DEQUE: ", dq)
