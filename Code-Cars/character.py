...
class Character:
	...
	def redraw_position_colour(self, rgbColourTuple = (0, 255, 0)):
		pygame.draw.rect(self.screen, rgbColourTuple, [self.border_width+(self.side_length+self.border_width)*self.current_position[0],\
			self.border_width+(self.side_length+self.border_width)*self.current_position[1], self.side_length, self.side_length])
	...
	# When the wrong code block is being executed 

	def shake_character(self):
		for i in range(3):
			sleep(0.25)
			self.redraw_position_colour((255,0,0))
			pygame.display.update()
			sleep(0.25)
			self.redraw_position_colour()
			pygame.display.update()
		sleep(0.25)

	def retrace_character(self):
		return
